-- Migration: Update create_walkin_order to support metadata (for tax/subtotal)
-- Adds p_metadata JSONB parameter and inserts into the metadata column.
...
CREATE OR REPLACE FUNCTION create_walkin_order(
    ...
    p_metadata JSONB DEFAULT '{}'::jsonb  -- NEW PARAMETER
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    INSERT INTO orders (
        ...
        p_metadata,  -- NEW: use the parameter
        NOW()
    )
