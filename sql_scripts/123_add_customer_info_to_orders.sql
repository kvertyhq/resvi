-- Migration 123: Add customer_name and customer_phone to orders table and update RPC
-- This ensures that customer details are snapshotted at time of order for reliable receipts.

-- Step 1: Add missing columns to the orders table
ALTER TABLE orders ADD COLUMN IF NOT EXISTS customer_name TEXT;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS customer_phone TEXT;

-- Step 2: Update the create_walkin_order RPC to include these columns
...
BEGIN
    ...
    IF v_user_final_id IS NULL AND v_norm_phone IS NOT NULL THEN
        ...
        IF v_user_final_id IS NULL THEN
            INSERT INTO profiles (full_name, phone, restaurant_id, created_at, updated_at)
            VALUES (COALESCE(p_customer_name, 'Guest'), p_customer_phone, p_restaurant_id, NOW(), NOW())
    ...
    ELSIF v_user_final_id IS NULL AND p_customer_name IS NOT NULL THEN
        ...
        INSERT INTO profiles (full_name, restaurant_id, created_at, updated_at)
        VALUES (p_customer_name, p_restaurant_id, NOW(), NOW())
        RETURNING id INTO v_user_final_id;
    END IF;
