-- 1. Add new columns to restaurant_settings
ALTER TABLE "public"."restaurant_settings"
ADD COLUMN IF NOT EXISTS "admin_notifications_phone" text DEFAULT NULL,
ADD COLUMN IF NOT EXISTS "sms_sender_id" text DEFAULT 'ResVi';
...
BEGIN
  ...
  -- verify restaurant exists and get SMS prefs and config
  select sms_preferences, sms_sender_id, admin_notifications_phone 
  into v_sms_prefs, v_sender_id, v_admin_phone
  ...
  -- Insert notification with expanded payload
...
-- 3. Update handle_booking_update_sms Trigger Function
...
DECLARE
    ...
    v_type TEXT;
BEGIN
    -- Fetch preferences and config
    SELECT sms_preferences, sms_sender_id, admin_notifications_phone 
    INTO v_sms_prefs, v_sender_id, v_admin_phone
    FROM restaurant_settings WHERE id = NEW.restaurant_id;
...
-- 4. Update handle_order_sms Trigger Function
...
BEGIN
    ...
    SELECT sms_preferences, sms_sender_id, admin_notifications_phone 
    INTO v_sms_prefs, v_sender_id, v_admin_phone
    FROM restaurant_settings WHERE id = v_restaurant_id;
    ...
    ELSIF TG_OP = 'UPDATE' AND OLD.status != NEW.status THEN
        ...
        ELSIF NEW.status = 'completed' THEN
            IF NEW.order_type = 'collection' THEN
                v_type := 'order_ready_collection'; 
            ELSE
                 v_type := 'order_completed_delivery'; 
            END IF;
