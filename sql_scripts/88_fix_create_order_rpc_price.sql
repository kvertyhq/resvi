-- Migration: Fix create_order_by_phone to respect client-provided price snapshot
-- This prevents the server from recalculating the price strictly from the base `menu_items` table,
-- which would ignore any price variants (sizes) or modifiers chosen by the customer.
...
begin
  ...
  for v_item in select * from jsonb_to_recordset(p_items) as x(menu_item_id uuid, quantity int, modifiers jsonb, selected_addons jsonb, price numeric)
  loop
    ...
    v_menu_price := coalesce(v_item.price, 0);
