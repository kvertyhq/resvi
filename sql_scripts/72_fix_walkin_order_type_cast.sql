-- Migration: Fix order_type cast in create_walkin_order RPC
-- The orders.order_type column is a custom enum (order_type), not TEXT.
-- This update casts p_order_type to the enum and sets default to 'dine_in'.
...
BEGIN
    ...
    IF p_total_amount <= 0 THEN
        RETURN json_build_object('success', false, 'error', 'Total amount must be greater than zero');
    END IF;
    ...
    INSERT INTO orders (
        ...
        p_order_type::order_type,  -- Cast TEXT to the order_type enum
        true,
        p_discount_type,
        p_discount_amount,
        p_order_source::order_source,  -- Cast TEXT to the order_source enum
        NOW()
    )
